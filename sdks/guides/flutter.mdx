...
1. 콘솔에서 다운로드 받은 **.env**파일을 lib/.env에 옮깁니다. 또는 **.env**파일을 생성한 후 아래와 같이 환경변수를 설정합니다. 만약.env 파일을 사용할 수 없다면 main.dart 파일의 clientId에 콘솔에서 가져온 Client ID를 넣습니다.
...
<Note>ios/Runner/info.plist 파일에 {app.scheme}를 추가해야합니다.</Note>
